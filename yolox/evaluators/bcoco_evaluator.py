...
class COCOEvaluator:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        if len(data_dict) > 0:
            ...
            precision_per_class, recall_per_class = self.calculate_per_class_precision_recall(cocoEval)

        # Add per-class precision and recall to the summary
            per_class_info = ""
            for i, name in enumerate(cat_names):
                per_class_info += f"\nClass: {name}, Precision: {precision_per_class[i]}, Recall: {recall_per_class[i]}"
    ...
    def calculate_per_class_precision_recall(self, cocoEval):
        precisions = cocoEval.eval["precision"]
        recalls = cocoEval.eval["recall"]

        precision_per_class = np.nanmean(precisions[:, :, :, 0, -1], axis=1) * 100
        recall_per_class = np.nanmean(recalls[:, :, 0, -1], axis=0) * 100
